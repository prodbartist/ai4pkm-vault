...
4. **볼트 열기**: 1단계 옵시디언 연동 시 `open-vault.sh`로 볼트를 자동으로 열어준다. 사용자가 별도 요청할 필요 없다.
...
- (선택) `jq` 설치: 볼트 자동 열기에 필요. `brew install jq` (macOS) / `sudo apt install jq` (Linux)
...
- **볼트 자동 열기** (사용자 요청 없이 즉시 실행):
  1. `_Settings_/Skills/gobi-onboarding/open-vault.sh "$(pwd)"` 실행
     - 스크립트가 볼트 등록 여부를 자동 확인하고, 미등록 시 자동 등록 후 열기
     - 성공 시: "옵시디언에서 볼트를 열었어요!"
     - jq 미설치 시: "볼트를 자동으로 여는 데 필요한 도구(jq)가 설치되어 있지 않아요. `brew install jq`로 설치하거나, 옵시디언에서 직접 'Open folder as vault'를 선택해주세요."
     - 옵시디언 미설치 시: "옵시디언이 설치되어 있지 않은 것 같아요. [obsidian.md/download](https://obsidian.md/download)에서 설치할 수 있어요."
  2. **CLI 연결 확인**: `obsidian version`으로 CLI 작동 확인. 실패 시 "옵시디언이 실행 중인지 확인해주세요. CLI는 옵시디언이 켜져 있어야 작동해요."
...
##### A. 체험
- 사용자: "해볼게요" / "네"
- 에이전트: "좋아요! 커뮤니티 스페이스에 참여하고, 첫 브레인 업데이트를 게시하고, 다른 사람들의 이야기도 살펴볼 거예요."
...
### 5-1. 스페이스 참여 (`gobi space warp`)

> 커뮤니티 스페이스에 참여하면 다른 전문가들과 지식을 나눌 수 있어요.
> 스페이스는 주제별 커뮤니티 공간이라고 생각하시면 돼요.
> 지금 참여할 수 있는 스페이스를 보여드릴게요.

`gobi space warp` 실행 → 사용자가 스페이스를 선택한다.
...
### 5-2. 브레인 업데이트 게시 (`gobi brain post-update`)

> 온보딩 하면서 프로필도 만들고 글도 써봤잖아요. 이걸 바탕으로 첫 브레인 업데이트를 게시해볼까요?
> 브레인 업데이트는 내 브레인 소식을 커뮤니티에 공유하는 거예요. 짧은 인사나 관심사 소개면 충분해요.

에이전트가 온보딩 중 만든 콘텐츠(프로필, writeup 등)를 바탕으로 제목/내용 초안을 작성하여 제시한다.

> 이런 내용은 어떨까요?
> 제목: "[사용자 이름]의 첫 브레인 업데이트"
> 내용: "[관심 분야]에 관심이 많은 [직업]입니다. 고비 데스크탑으로 지식 관리를 시작했어요. 잘 부탁드려요!"
> 이대로 게시할까요, 아니면 수정하실 부분이 있으세요?

#### 사용자 응답 시나리오

##### A. 게시 동의
- 사용자: "좋아요, 게시해주세요" / "이대로 할게요"
- `gobi brain post-update --title "제목" --content "내용"` 실행
- 에이전트: "첫 브레인 업데이트가 게시됐어요! 스페이스에서 다른 사람들이 볼 수 있어요."

##### B. 내용 수정 요청
- 사용자: "내용을 좀 바꾸고 싶어요" / "[수정 내용]으로 바꿔주세요"
- 에이전트: (수정 반영 후) "이렇게 수정했어요. 게시할까요?"
- 사용자 확인 후 `gobi brain post-update --title "수정된 제목" --content "수정된 내용"` 실행

##### C. 나중에/건너뛰기
- 사용자: "나중에 할게요" / "건너뛸게요"
- 에이전트: "알겠어요. 나중에 '브레인 업데이트'라고 말하면 언제든 도와드릴게요. 다음으로 넘어갈게요."

### 5-3. 최신 쓰레드 확인 (`gobi space list-threads`)

> 스페이스에서 어떤 대화가 오가는지 한번 볼까요?
> 쓰레드는 스페이스 안의 게시글이나 대화라고 생각하시면 돼요.
> 최근 이야기들을 보여드릴게요.

`gobi --json space list-threads --limit 5` 실행 → 결과를 요약하여 사용자에게 제시한다.

> 최근 스페이스에서는 이런 이야기들이 오가고 있어요.
> (쓰레드 제목/내용 요약 3~5개 제시)
> 관심 가는 주제가 있으세요?

#### 사용자 응답 시나리오

##### A. 관심 표현
- 사용자: "이거 재밌네요" / "[주제]에 관심 있어요"
- 에이전트: "좋아요! 나중에 이 주제에 대해 더 알아보거나 직접 대화에 참여할 수도 있어요."

##### B. 질문
- 사용자: "이건 뭐예요?" / "어떻게 참여해요?"
- 에이전트: (질문에 맞춰 답변 후) "스페이스에서 자유롭게 대화할 수 있어요. 관심 있는 쓰레드에 댓글을 달거나 새 글을 올릴 수도 있어요."

##### C. 건너뛰기
- 사용자: "됐어요" / "넘어갈게요"
- 에이전트: "알겠어요. 나중에 언제든 스페이스를 탐색할 수 있어요."

### 완료 확인 & 전환

> 다섯 번째 단계 완료! 커뮤니티 스페이스에 참여하고 지식을 공유하는 방법을 알아봤어요.
> 마지막으로 온보딩을 마무리할게요.
...
| v3.2 | 2026-03-05 | 옵시디언 자동 볼트 열기: open-vault.sh 스크립트 추가, 수동 'Open folder as vault' 대체 |
