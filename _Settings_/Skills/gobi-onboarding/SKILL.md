...
| 인덱스 | 자동 정리/목록 | 도서관 목록 | Post-boarding |
| DRB | 아침 뉴스 브리핑 | 매일 아침 관심 주제 뉴스 모음 | Post-boarding |
...
온보딩 이후 추가로 안내하는 항목들이다. 온보딩 완료 후 또는 사용자가 요청할 때 안내한다.
...
| Post-boarding: GDR 수동 실행 후 | GDR | `_Settings_/Tasks/*GDR*` → `AI/Roundup/` 출력 |
| Post-boarding: TIU 수동 실행 후 | TIU | `_Settings_/Tasks/*TIU*` → `Topics/` 출력 |
