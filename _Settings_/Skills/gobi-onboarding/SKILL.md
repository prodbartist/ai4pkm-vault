...
> 다음은 프로필을 만들 거예요. 어떤 분인지 파악해서 맞춤으로 도와드릴게요.
...
> 시작하기 전에, 혹시 옵시디언을 사용하시거나 관심 있으세요?
> 옵시디언을 연결하면 지식 그래프를 직접 탐색하고, 웹 클리퍼로 자료를 더 편하게 가져올 수 있어요.
...
- (`obsidian_enabled = false`로 설정, 기존 플로우 그대로 진행)

**상태 추적**: `obsidian_enabled`는 대화 컨텍스트에서 추적한다. 세션 재개 시 ("온보딩 계속") `.obsidian/` 폴더 존재 여부로 판단한다.
...
**`obsidian_enabled = true`인 경우**: 옵시디언과 웹 클리퍼를 이미 3단계에서 설정했으므로, Post-boarding 옵시디언/웹 클리퍼 섹션을 건너뛰고 바로 클로징한다.
