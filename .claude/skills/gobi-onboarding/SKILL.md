---
name: gobi-onboarding
description: 고비 데스크탑 3.0 대화형 음성 온보딩 가이드
metadata:
  version: 2.0.0
  author: lifidea
  created: 2026-02-04
  updated: 2026-02-05
---

## When to Use

이 스킬은 사용자가 온보딩을 요청할 때 활성화한다.

### 트리거 키워드
- "온보딩 시작" / "온보딩 계속" / "이어서 하자"
- "onboarding" / "시작 도우미"

### 참고
- 음성 출력 규칙은 RVA 시스템 프롬프트에서 이미 적용되므로, 이 스킬에서는 온보딩 플로우만 담당한다.

## 온보딩 철학

**"클릭 수 최소화, 가치 최대화"**

1. 사용자의 기존 환경을 먼저 파악하고 적응한다
2. 연결하면 자동으로 정리되어야 한다
3. 커뮤니티 참여로 지식 공유의 가치를 체험한다
4. 필수만 먼저, 나머지는 Post-boarding으로 점진적 공개

사용자는 복잡한 개념을 배우는 게 아니라, 가치를 체험하는 것이다. 각 단계마다 "이걸 하면 이런 좋은 일이 생겨요"를 먼저 보여주고, 행동은 최대한 단순하게 안내한다.

## Pre-boarding (온보딩 진입 조건)

온보딩을 시작하기 전에 아래 조건이 완료되어 있어야 한다. 이 단계들은 온보딩 플로우에 포함되지 않으며, 별도로 안내한다.

- 고비 데스크탑 설치 완료
- 구글 계정으로 로그인 완료
- 메인 볼트 생성 완료

### Pre-boarding 미완료 시 대응

- 사용자: "온보딩 시작할게요" (볼트 미생성 상태)
- 에이전트: "온보딩을 시작하려면 먼저 볼트를 만들어야 해요. 볼트는 내 지식을 담는 공간이에요. 새 볼트 만들기를 눌러서 하나 만들어주세요. 만들고 나면 다시 '온보딩 시작'이라고 말해주세요."

## 오프닝 인사

### 에이전트 웰컴 스크립트

> 안녕하세요! 온보딩을 시작할게요.
> 지금부터 네 단계로 진행되는데, 필수 단계만 하면 십 분이면 충분해요.
> 먼저 어떤 분인지 파악하고, 자료를 연결하고, 자동 정리를 켜고, 커뮤니티를 설정하는 순서예요.
> 바로 시작할까요?

### 사용자 응답 시나리오

#### A. 바로 시작
- 사용자: "네" / "시작할게요"
- 에이전트: "좋아요! 그럼 첫 번째 단계부터 시작할게요."

#### B. 먼저 설명 요청
- 사용자: "이게 뭔가요?" / "먼저 설명해주세요"
- 에이전트: "고비 데스크탑은 내 지식을 모으고, 정리하고, 공유할 수 있는 도구예요. 메모, 파일, 메일 같은 걸 한 곳에 모아두면, 에이아이가 자동으로 정리해주고, 나중에 뭐든 물어보면 내 자료를 바탕으로 답변해줘요. 준비되시면 시작할까요?"

#### C. 일부 이미 완료
- 사용자: "이미 인제스트는 했어요"
- 에이전트: "아, 벌써 진행하셨군요! 어디까지 하셨는지 확인해볼게요. 잠시만요."
- (진행 상태 확인 후 해당 단계부터 이어서 안내)

## 1단계: 사용자 파악 & 인제스트

### 에이전트 오프닝

> 첫 번째 단계는 자료 연결이에요.
> 먼저 어떤 분인지 파악해서, 맞춤으로 연결해드릴게요.
> 혹시 개인 홈페이지나 블로그가 있으세요?

### 1-1. 사용자 파악

> (홈페이지/블로그가 있는 경우) 주소를 알려주시면 자동으로 가져와서 프로필을 만들어드릴게요.
> (없는 경우) 괜찮아요. 어떤 분야에서 일하시는지, 관심사가 뭔지 간단히 알려주세요. 프로필을 같이 만들어봐요.

#### 사용자 응답 시나리오

##### A. 홈페이지 있음
- 사용자: "제 블로그 주소는 example.com이에요"
- 에이전트: "좋아요! 지금 가져오고 있어요. 잠시만 기다려주세요." (백그라운드 처리, 완료 시 알림)
- 에이전트: "프로필이 만들어졌어요! 화면에서 확인해보세요. 수정할 부분이 있으면 말씀해주세요."

##### B. 홈페이지 없음
- 사용자: "없어요"
- 에이전트: "괜찮아요. 어떤 일을 하시는지, 또는 관심 있는 분야가 뭔지 알려주시면 제가 프로필 초안을 만들어드릴게요."

### 1-2. 정보 소스 문의

> 프로필이 준비됐으니, 이제 평소에 어떤 정보를 많이 쓰시는지 알려주세요.
> 어떤 정보를 볼트에 넣고 싶으세요?

#### 사용자 응답 시나리오

##### A. 구글에 정보가 많은 사용자
- 사용자: "메일이랑 캘린더를 주로 써요" / "구글 드라이브에 자료가 많아요"
- 에이전트: "그럼 구글 워크스페이스를 연결하면 딱이에요. 캘린더, 메일, 드라이브를 한번에 연결할 수 있어요. 구글 연동 설정을 열어드릴까요?"
- (Google Workspace 세팅 안내)

##### B. 노트 앱을 사용하는 사용자
- 사용자: "애플 노트를 쓰고 있어요" / "노션에 자료가 있어요"
- 에이전트: "기존 노트를 가져올 수 있는 도구가 있어요. 옵시디언 임포터라고 하는데, 애플 노트나 노션에서 자료를 한번에 가져올 수 있어요. 지금 연결해볼까요?"
- (Obsidian Importer 안내)

##### C. 웹 중심 사용자
- 사용자: "웹에서 자료를 많이 모아요" / "아티클 읽는 걸 좋아해요"
- 에이전트: "그럼 웹 클리핑이 딱이에요. 관심 있는 웹페이지 주소를 저장하면 `Ingest/Clippings` 폴더에 원본 마크다운이 저장돼요. 그러면 이아이씨 워커가 자동으로 요약하고 정리해서 `AI/Summary`에 넣어줘요. 피케이엠 관련 좋은 아티클로 먼저 연습해볼까요?"
- 에이전트: "세컨드 브레인이라는 개념에 대한 좋은 글들이 있어요. 하나 골라서 클리핑해볼까요?"
- (아래 예시 아티클 중 하나를 제시하여 클리핑 체험 유도)
- (Obsidian Web Clipper는 Post-boarding에서 추가 소개)

**예시 아티클 (세컨드 브레인 주제):**
- [Building a Second Brain: The Definitive Introductory Guide](https://fortelabs.com/blog/basboverview/) - Tiago Forte
- [The PARA Method: The Simple System for Organizing Your Digital Life](https://fortelabs.com/blog/para/) - Tiago Forte
- [두 번째 뇌를 만드는 10가지 원칙](https://brunch.co.kr/@analysisman/7) - 분석맨
- [제텔카스텐의 네 가지 기본 원칙](https://brunch.co.kr/@analysisman/34) - 분석맨

##### D. 문서가 있는 사용자
- 사용자: "파일이 있어요" / "문서를 넣고 싶어요" / "PDF가 있어요"
- 에이전트: "좋아요! 문서를 바로 넣을 수 있어요. 지금 문서 폴더를 열어드릴게요."
- (Finder에서 `Ingest/Documents` 폴더를 열어줌: `obsidian://open?vault=ai4pkm-vault&file=Ingest%2FDocuments`)
- 에이전트: "이 폴더에 파일을 드래그해서 넣어주세요. 피디에프, 워드, 이펍 같은 문서를 넣으면 이디엠이 자동으로 마크다운으로 변환해줘요."
- (EDM 프롬프트가 자동으로 마크다운 변환 처리)

##### E. 잘 모르겠는 사용자
- 사용자: "뭘 넣어야 할지 모르겠어요"
- 에이전트: "괜찮아요. 가장 쉬운 건 폴더 연결이에요. 컴퓨터에 있는 문서 폴더 하나만 연결하면 돼요. 나중에 더 추가할 수 있으니까 부담 없이 시작해봐요."

##### F. 혼란
- 사용자: "인제스트가 뭐예요?"
- 에이전트: "인제스트는 '가져오기'라고 생각하시면 돼요. 여기저기 흩어진 내 자료를 한 곳에 모으는 거예요. 한번 연결해두면 새 파일이 생길 때마다 알아서 가져와요."

##### G. 건너뛰기
- 사용자: "나중에 할게요"
- 에이전트: "최소 하나는 연결해야 에이아이가 도움을 줄 수 있어요. 가장 쉬운 건 폴더 연결이에요. 문서 폴더 하나만 골라보시겠어요?"

##### H. 추가 질문
- 사용자: "내 자료가 외부로 나가나요?"
- 에이전트: "아니요, 자료는 내 컴퓨터에 그대로 있어요. 고비가 그 파일들을 읽어서 정리해주는 것뿐이에요."
- 사용자: "여러 개 연결할 수 있나요?"
- 에이전트: "네, 원하는 만큼 연결할 수 있어요. 지금은 하나만 하고, 나중에 더 추가해도 돼요."

### 완료 확인 & 전환

> 첫 번째 단계 완료! 자료 연결이 잘 됐어요.
> 다음은 자동 정리 설정이에요. 매일 자료가 알아서 정리되게 해줄 거예요.

## 2단계: 인덱스 셋업

### 에이전트 오프닝

> 두 번째 단계는 인덱스 셋업이에요.
> 연결한 자료를 에이아이가 자동으로 정리해주는 거예요.
> 두 가지 옵션이 있는데, 하나는 기본으로 켜져 있어요.

### 실행 가이드

> 첫째, 데일리 라운드업. 매일 새로 들어온 자료를 자동으로 요약 정리해줘요. 이건 기본으로 켜져 있어요.
> 둘째, 토픽 인덱스. 주제별로 자동 분류해줘요. 이건 선택이에요.
> 토픽 인덱스가 어떤 건지 이미지로 보여드릴게요.

![[_Settings_/Prompts/_files_/graph-example.png]]

> 이렇게 주제별로 연결된 지식 그래프가 만들어져요.
> 토픽 인덱스에 어떤 주제가 가능한지 보여드릴까요?

### 사용자 응답 시나리오

#### A. 토픽 인덱스도 설정
- 사용자: "네, 보여주세요" / "토픽 인덱스도 할게요"
- 에이전트: "연결한 자료를 분석해보니 이런 주제들이 있어요." (가능한 토픽 목록 표시)
- 에이전트: "이 중에서 자동 정리할 주제를 골라주세요. 여러 개 골라도 돼요."

#### B. 데일리만 사용
- 사용자: "데일리만 할게요" / "기본만 쓸게요"
- 에이전트: "좋아요! 데일리 라운드업이 매일 자동으로 돌아갈 거예요. 토픽 인덱스는 나중에 언제든 켤 수 있어요."

#### C. 혼란
- 사용자: "인덱스가 뭐예요?" / "왜 필요해요?"
- 에이전트: "인덱스는 도서관의 목록이라고 생각하시면 돼요. 자료를 그냥 쌓아두면 찾기 어렵잖아요. 인덱스가 있으면 '이 주제 자료 보여줘' 하면 바로 찾아줘요."

#### D. 건너뛰기
- 사용자: "나중에 할게요"
- 에이전트: "데일리 라운드업은 기본으로 켜져 있으니 신경 쓰실 필요 없어요. 토픽 인덱스는 나중에 추가하셔도 돼요."

#### E. 추가 질문
- 사용자: "자동으로 돌아가나요?"
- 에이전트: "네, 한번 설정하면 매일 자동으로 돌아가요. 새 자료가 추가되면 알아서 분류하고 정리해줘요."

### 2-1. DRB Worker 제안

> 하나 더 추천해드릴 게 있어요.
> 매일 아침 다섯 시에 관심 주제의 최신 뉴스와 논문을 자동으로 찾아서 브리핑을 만들어줄 수 있어요.
> 디알비 워커라고 하는데, 켜볼까요?

#### 사용자 응답 시나리오

##### A. 활성화
- 사용자: "네, 켜주세요" / "좋아요"
- 에이전트: "좋아요! 디알비 워커를 켰어요. 내일 아침에 첫 브리핑이 준비될 거예요."

##### B. 거절/나중에
- 사용자: "나중에요" / "안 할래요"
- 에이전트: "알겠어요. 나중에 설정에서 언제든 켤 수 있어요."

##### C. 혼란
- 사용자: "디알비가 뭐예요?"
- 에이전트: "데일리 리서치 브리핑의 줄임말이에요. 내가 관심 있는 주제의 최신 소식을 매일 자동으로 모아서 정리해주는 거예요."

### 완료 확인 & 전환

> 두 번째 단계 완료! 이제 자료가 자동으로 정리돼요.
> 다음은 커뮤니티 설정이에요. 다른 전문가들과 지식을 나눌 수 있는 프로필을 만들 거예요.

## 3단계: 커뮤니티 셋업

### 에이전트 오프닝

> 세 번째 단계는 커뮤니티 셋업이에요.
> 에이포피케이엠 커뮤니티에서 다른 전문가들과 지식을 나눌 수 있어요.
> 아까 만든 프로필을 바탕으로 커뮤니티 프로필을 만들어볼게요.

### 3-1. 커뮤니티 프로필 생성

> 아까 파악한 정보를 바탕으로 커뮤니티 프로필 초안을 만들었어요.
> 화면에서 확인해보세요. 수정할 부분이 있으면 말씀해주세요.

#### 사용자 응답 시나리오

##### A. 순조로운 진행
- 사용자: "좋아요" / "괜찮아요"
- 에이전트: "완벽해요! 프로필이 저장됐어요."

##### B. 수정 요청
- 사용자: "이 부분 바꿔주세요"
- 에이전트: "어떻게 바꿀까요? 알려주시면 바로 수정할게요."

##### C. 혼란
- 사용자: "커뮤니티가 뭐예요?"
- 에이전트: "같은 관심사를 가진 전문가들이 모여서 지식을 나누는 공간이에요. 내 전문 지식을 공유하고, 다른 사람의 지식도 접할 수 있어요."

### 3-2. 공유 주제 선택

> 커뮤니티에서 어떤 주제로 지식을 공유하고 싶으세요?
> 관심 분야나 전문 영역을 알려주시면 관련 주제를 추천해드릴게요.

#### 사용자 응답 시나리오

##### A. 주제 선택
- 사용자: "에이아이 관련 주제요" / "마케팅이요"
- 에이전트: "좋아요! 관련 주제를 설정했어요. 이 주제로 커뮤니티에서 활동할 수 있어요."

##### B. 잘 모르겠는 사용자
- 사용자: "뭘 골라야 할지 모르겠어요"
- 에이전트: "괜찮아요. 인제스트한 자료를 보니 이런 주제들이 있어요. 이 중에 관심 있는 게 있으세요?"
- (인제스트 기반 주제 추천)

### 3-3. 추가 인제스트 확인

> 커뮤니티 프로필이 만들어졌어요!
> 혹시 커뮤니티에 공유하기 위해 추가로 넣고 싶은 자료가 있나요?

#### 사용자 응답 시나리오

##### A. 추가 자료 있음
- 사용자: "네, 이 폴더도 추가할게요"
- 에이전트: "좋아요! 추가하고 있어요." (백그라운드 처리)

##### B. 추가 자료 없음
- 사용자: "없어요" / "나중에 할게요"
- 에이전트: "알겠어요. 나중에 언제든 추가할 수 있어요. 다음 단계로 넘어갈게요."

### 완료 확인 & 전환

> 세 번째 단계 완료! 커뮤니티 프로필이 준비됐어요.
> 거의 다 왔어요! 마지막 단계에서 마무리할게요.

## 4단계: 마무리 & 첫 체험

### 에이전트 오프닝

> 마지막 네 번째 단계예요!
> 지금까지 설정한 걸 확인하고, 에이아이에게 직접 질문해보는 시간이에요.

### 실행 가이드

> 이제 볼트에 자료가 들어가고 있어요.
> 한번 질문을 해볼까요? 아무거나 좋아요. 예를 들어 '내가 어떤 자료를 넣었지?'라든가, 관심 분야에 대해 물어보셔도 돼요.

### 사용자 응답 시나리오

#### A. 순조로운 진행
- 사용자: (질문을 하고 답변을 받음) "오, 신기하다!"
- 에이전트: "맞죠? 내가 모아둔 자료에서 답변을 찾아주는 거예요. 자료가 쌓일수록 답변이 더 정확해져요."

#### B. 혼란
- 사용자: "뭘 물어봐야 해요?"
- 에이전트: "아무거나 좋아요. 예를 들어 '지난주에 뭐 했지?'라든가, '이 주제에 대해 내가 뭘 알고 있지?' 같은 것도 좋아요. 편하게 물어보세요."

#### C. 건너뛰기
- 사용자: "나중에 해볼게요"
- 에이전트: "알겠어요. 언제든 대화창에서 질문하시면 돼요."

#### D. 오류 발생
- 사용자: "답변이 이상해요" / "관련 없는 답이 나와요"
- 에이전트: "처음에는 자료가 적어서 그럴 수 있어요. 자료를 더 연결하고 시간이 지나면 점점 정확해져요."

### 완료 확인 → 온보딩 완료

> 네 단계를 모두 완료했어요! 수고하셨어요.

## 온보딩 완료

### 클로징 스크립트

> (전체 완료) 축하해요! 온보딩을 모두 완료하셨어요!
> 프로필을 만들고, 자료를 연결하고, 자동 정리를 켜고, 커뮤니티까지 설정하셨어요.
> 이제 고비 데스크탑이 매일 나를 도와줄 준비가 됐어요.

> (일부 건너뛴 경우) 수고하셨어요! 필수 단계는 모두 완료했어요.
> 건너뛴 단계는 나중에 설정에서 언제든 할 수 있어요.

### Post-boarding 안내

> 추가로 설정하면 좋은 것들이 있어요.
> 첫째, 옵시디언을 설치하면 볼트를 더 편리하게 관리할 수 있어요. 고비 데스크탑만으로도 사용할 수 있지만, 옵시디언이 있으면 노트를 직접 편집하고 연결하기가 훨씬 좋아요.
> 둘째, 옵시디언 웹 클리퍼를 설치하면 웹 브라우저에서 바로 관심 있는 글을 볼트에 저장할 수 있어요.
> 웹 클리퍼 설정 화면을 보여드릴게요.

![[_Settings_/Prompts/_files_/owc-config.png]]

> 관심 있으시면 '옵시디언 설치 알려줘'라고 말해주세요.

### 다음 단계 제안

> 이제 뭘 하면 좋을까요? 두 가지를 추천해요.
> 첫째, 내일 아침에 데일리 라운드업을 확인해보세요. 오늘 연결한 자료가 정리돼 있을 거예요.
> 둘째, 커뮤니티를 둘러보세요. 관심 분야의 다른 전문가들을 만날 수 있어요.

### 지속 지원 안내

> 언제든 도움이 필요하면 '도움'이라고 말해주세요.
> 궁금한 게 있으면 뭐든 물어보세요. 항상 여기 있을게요.
> 고비와 함께 즐거운 지식 여행 되세요!

## 세션 관리

### 중단 & 재개
- 사용자: "나중에 할게요" / "그만할래요"
- 에이전트: "알겠어요. 지금까지 N단계까지 완료했어요. 나중에 '온보딩 계속'이라고 말씀하시면 여기서 이어서 할 수 있어요. 수고하셨어요!"

- 사용자: "온보딩 계속" / "이어서 하자"
- 에이전트: "다시 오셨군요! 지난번에 N단계까지 완료하셨어요. N+1단계부터 이어서 할게요. 준비되셨나요?"

### 이전 단계로 돌아가기
- 사용자: "돌아가기" / "이전 단계로"
- 에이전트: "어떤 단계로 돌아갈까요? 완료한 단계를 알려드릴게요."
- (완료 단계 목록 제시 후 선택)

### 진행 상황 확인
- 사용자: "어디까지 했어?" / "진행 상황"
- 에이전트: "지금까지의 진행 상황이에요." (프로그레스 바 표시와 함께 안내)

### 도움 요청
- 사용자: "도움" / "뭘 할 수 있어?"
- 에이전트: "사용할 수 있는 명령이 있어요. '계속'하면 다음 단계로, '건너뛰기'는 선택 단계를 넘기고, '다시'는 현재 단계를 다시 하고, '돌아가기'는 이전으로, '진행 상황'은 어디까지 했는지, '중단'은 나중에 이어서 하는 거예요."

## 이미지 표시 가이드

온보딩 중 아래 이미지를 적절한 시점에 사용자에게 보여준다. 이미지는 `_Settings_/Prompts/_files_/`에 저장되어 있다.

| 이미지 | 표시 시점 | 용도 |
|--------|----------|------|
| `graph-example.png` | 2단계 토픽 인덱스 설명 시 | 지식 그래프 예시로 인덱스의 가치를 시각적으로 시연 |
| `owc-config.png` | Post-boarding 웹 클리퍼 안내 시 | 웹 클리퍼 설정 화면을 보여줘서 설치 진입장벽을 낮춤 |
| `pbc-canvas.png` | 브레인스톰 기능 안내 시 (참고용) | 패시브 브레인스톰 캔버스 결과 예시 |

## 부록

### 단축 명령어

| 명령어 | 동작 |
|--------|------|
| 계속 | 다음 단계로 진행 |
| 건너뛰기 | 현재 단계 건너뛰기 (선택 단계만) |
| 다시 | 현재 단계 처음부터 |
| 돌아가기 | 이전 단계 선택 |
| 진행 상황 | 완료/현재/남은 단계 표시 |
| 도움 | 사용 가능한 명령어 안내 |
| 중단 | 진행 상태 저장 후 종료 |

### 용어 사전

| 용어 | 뜻 | TTS 발음 |
|------|-----|----------|
| PKM | Personal Knowledge Management, 개인 지식 관리 | 피케이엠 |
| Vault | 지식 저장소, 볼트 | 볼트 |
| Ingest | 자료 가져오기, 인제스트 | 인제스트 |
| Index | 자동 정리, 인덱스 | 인덱스 |
| Daily Roundup | 매일 자동 요약 정리 | 데일리 라운드업 |
| Topic Index | 주제별 자동 분류 | 토픽 인덱스 |
| Community | 전문가 지식 공유 공간 | 커뮤니티 |
| Q&A | 질문과 답변 | 큐앤에이 |
| OAuth | 구글 로그인 인증 방식 | 오어스 |
| Gemini | 구글 에이아이 백엔드 | 제미나이 |
| Obsidian Importer | 다른 앱 노트 가져오기 도구 | 옵시디언 임포터 |
| Web Clipper | 웹페이지 저장 브라우저 확장 | 웹 클리퍼 |
| DRB | Daily Research Briefing, 매일 자동 리서치 브리핑 | 디알비 |
| EDM | Extract Document to Markdown, 문서 마크다운 변환 | 이디엠 |

### 학습 이력

(사용자 피드백에 따라 업데이트)
